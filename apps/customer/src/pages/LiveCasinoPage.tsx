...
import {
  liveCasinoGamesData,
  liveCasinoProviders,
} from "../data/liveCasinoData";

import { PlayCircle, Filter } from "lucide-react";
import { LiveCasinoCard } from "../components/cards/LiveCasinoCard";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-zinc-800 pb-6">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1">
            Real-time HD streaming studios with professional human dealers.
          </p>
        </div>
      ...
      {filteredGames.length > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {filteredGames.map((game) => (
            <LiveCasinoCard key={game.id} game={game} />
          ))}
        </div>
      ) : (
        <div className="text-center py-16 bg-[#141414] rounded-xl border border-[#222]">
          <p className="text-sm text-zinc-400">
            No live tables found for {selectedProvider}.
          </p>
        </div>
      )}
    </div>
