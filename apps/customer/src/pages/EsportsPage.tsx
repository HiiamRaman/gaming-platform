...
import { esportsGamesData, esportsProviders } from "../data/esportsData";
import { EsportsCard } from "../components/EsportsCard";
import { Gamepad2, Filter } from "lucide-react";

export const EsportsPage: React.FC = () => {
  const [selectedProvider, setSelectedProvider] = useState("All Providers");

  const filteredGames =
    selectedProvider === "All Providers"
      ? esportsGamesData
      : esportsGamesData.filter((game) => game.provider === selectedProvider);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-zinc-800 pb-6">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1">
            Real-time odds, major pro leagues, and competitive esports match
            lobbies.
          </p>
        </div>
      ...
      {filteredGames.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredGames.map((game) => (
            <EsportsCard key={game.id} game={game} />
          ))}
        </div>
      ) : (
        <div className="text-center py-16 bg-[#141414] rounded-xl border border-[#222]">
          <p className="text-sm text-zinc-400">
            No esports lobbies found for {selectedProvider}.
          </p>
        </div>
      )}
    </div>
